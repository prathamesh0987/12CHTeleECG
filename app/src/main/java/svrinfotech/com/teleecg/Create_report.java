...
public class Create_report extends View {
    ...
    @SuppressLint("SimpleDateFormat") public Create_report(String comment,String password,Context context)
    {
        super(context);
        p_context=context;
        ...
        else
        {
            ...
            if(MainActivity.create_pdf==true)
            {
                create_pdf();
                create_binary();
            }
    ...
    public void plot_data()
    {
        ...
        if(MainActivity.create_image==true)
            create_png();//creates PDF of ECG Report
        if(MainActivity.create_pdf==true /*&& png_created==true*/)
            create_pdf();//creates PDF of ECG Report

        if(MainActivity.create_text==true)
            create_ascii_file();
    }
    ...
    private void print_details_pdf(PdfContentByte canvas) {
        try {
            ...
            Phrase chno = new Phrase("CHSSNO           :", new Font(base_font, fontSize));
            ...
            Phrase date = new Phrase("Date                 :", new Font(base_font, fontSize));

...
public class paintView extends View
{
    ...
    int 						adj_height;
    public static String 		Pass_On_date,Pass_On_chno,Pass_On_name,Pass_On_age,Pass_On_gen,Pass_On_medi,Pass_On_BP,Pass_On_dob,Pass_On_ht,Pass_On_wt;
    ...
    public paintView(String date1, String sname, String schno,String sdob, String sage,String sgen,String sht,String swt,String smedi,String sBP,Context c)
    {
    ...
    protected void onDraw(Canvas canvas)
    {
        ...
        if( (!Auto && MainActivity.refresh_data))//should be changed for 12 Channels and manual mode
            refresh_canvas();
        else if((array_full  && Auto)|| MainActivity.refresh_data)//Auto mode
            refresh_canvas();
                    ...
                    if(ADS_samples_count%XAXIS_WD==0)
                    {
                        ...
                        if(iScreenWidth<=XAXIS_WD)	//for small size mobile
                        {
                            for(int l=(ADS_samples_count-(XAXIS_WD-avg_incmt)); l<=ADS_samples_count;l+=avg_incmt)
                            {
                                ...
                                List_arrayOf_v1.add(display_load_data[i][disp_count]);
                                disp_count++;
                            }
                        }
                        else
    ...
    public void initialise_array()
    {
        ...
        for(int i=0;i<CH_NUM;i++)
        {
            ...
            List_arrayOf_0[i]=new ArrayList<>();
    ...
    private void print_lead_name(int pg) {
        try {
            ...
            if(MainActivity.load_existing_file)//print lead name during load data
            {
    ...
    public String print_selected_filter(int pg) {
        ...
        try {
            ...
            String filter_txt_arr[]={"Filter:off","Filter:0 to 150 Hz","Filter:5 to 40 Hz","Filter:0 to 40 Hz","Filter:5 to 25 Hz","Filter:0 to 25 Hz"};
            ...
            if(MainActivity.load_existing_file)
            {
                ...
                for (int i = 0; i < 6; i++)
                    if(filter_state==i)
                        cnvs.drawText(filter_txt_arr[i], (iScreenWidth/2),(singleLeadHeight/5), paint);

                //print page number
                cnvs.drawText("pg:"+pg, (iScreenWidth)/4,(singleLeadHeight/5), paint);//3/4,6/7
            }//if(MainActivity.load_existing_file==true)
